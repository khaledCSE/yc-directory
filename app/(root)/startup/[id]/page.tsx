...
export const experimental_ppa = true;
...
const StartupPage = async ({ params }: { params: Promise<{ id: string }> }) => {
  ...
  // const post: Omit<Startup, "author"> & { author: Author } = await client.fetch(STARTUP_BY_ID_QUERY, { id })

  // const { select: editorsPicks } = await client.fetch(PLAYLIST_BY_SLUG_QUERY, { slug: 'editor-s-picks' })
        ...
        <div className="space-y-5 mt-10 max-w-4xl mx-auto">
          <div className="flex-between gap-5">
            <Link href={`/user/${post?.author?._id}`} className='flex gap-2 items-center mb-3'>
              <Image src={post?.author?.image ?? ''} alt='avatar' className='rounded-full' width={64} height={64} />
              <div>
        ...
        {editorsPicks?.length > 0 && (
          <div className="max-w-4xl mx-auto">
            ...
            <ul className="mt-7 card_grid-sm">
              {editorsPicks.map((post: StartupCardType, i: number) => (
